...
void remove_semaphores_from_list(select_t *select_item, sem_t *select_sem) {
...
void add_semaphores_to_list(select_t *select_item, sem_t *select_sem) {
channel_t* channel = select_item->channel;
pthread_mutex_lock(&channel->mutex);
...
enum channel_status channel_select(select_t* channel_list, size_t channel_count, size_t* selected_index)
{
	...
	for (size_t i = 0; i < channel_count; i++) {
		add_semaphores_to_list(&channel_list[i], &select_sem);
	}
	...
	for(;;) {
		for (size_t i = 0; i < channel_count; i++) {
			...
			*selected_index = i;
			...
			remove_semaphores_from_list(&channel_list[k], &select_sem);
			}
			...
			*selected_index = i;
			...
			sem_destroy(&select_sem);
			return CLOSED_ERROR; // send closed_error to upstream functions
			} else {
			pthread_mutex_unlock(&channel->mutex);
			}
			...
			// receive operation might have been successful.. clean up the list
			*selected_index = i;
			for (size_t j = 0; j < channel_count; j++) {
			remove_semaphores_from_list(&channel_list[j], &select_sem);
			}
			sem_destroy(&select_sem);
			return status;
			}
...
// if code comes here, it means channels did were not empty/full for send/recv and hence we need to wait (block)
sem_wait(&select_sem);
}
}
